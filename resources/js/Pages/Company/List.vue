<template>
  <app-layout>
    ...
    <!-- add -->
    <div class="flex">
      <btn :href="createUrl">Add company</btn>
    </div>
    ...
    >
      <table class="bg-white">
        ...
        <tbody>
          <tr class="group border-b border-blueGray-200" v-for="company in filtered" :key="company.id">
            <td class="px-2 py-0.5">{{ company.name }}</td>
            ...
            <td class="flex flex-row justify-end items-center space-x-1 px-2 py-0.5 min-h-2.25">
              <button
                ...
                @click="showModal(true, company.deleteUrl)"
              >
    ...
    <confirmation-modal :show="isOpenModal" @close="showModal(false)">
      ...
      <template #content>Are you sure you want to delete this company?</template>
      <template #footer>
        <btn variant="secondary" @click="showModal(false)">Cancel</btn>
        <btn class="ml-2" @click="deleteCompany">Delete</btn>
      </template>
    </confirmation-modal>
  </app-layout>
</template>
