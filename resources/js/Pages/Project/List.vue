<template>
  <app-layout>
    ...
    <!-- add -->
    <div class="flex">
      <btn :href="createUrl">Add Project</btn>
    </div>
    ...
    >
      <table class="bg-white">
        ...
        <tbody>
          <tr class="group border-b border-blueGray-200" v-for="project in filtered" :key="project.id">
            <td class="px-2 py-0.5">{{ project.name }}</td>
            <td class="px-2 py-0.5">n/a</td>
            <td class="px-2 py-0.5">Active</td>
            <td class="flex flex-row justify-end items-center space-x-1 px-2 py-0.5 min-h-2.25">
              <button
                ...
                @click="showModal(true, project.deleteUrl)"
              >
    ...
    <confirmation-modal :show="isOpenModal" @close="showModal(false)">
      ...
      <template #content>Are you sure you want to delete this project?</template>
      <template #footer>
        <btn variant="secondary" @click="showModal(false)">Cancel</btn>
        <btn class="ml-2" @click="deleteProject">Delete</btn>
      </template>
    </confirmation-modal>
  </app-layout>
</template>
