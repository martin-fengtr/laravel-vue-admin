<template>
  <app-layout>
    ...
    <!-- add -->
    <div class="flex">
      <btn :href="createUrl">Add User</btn>
    </div>
    ...
    >
      <table class="bg-white">
        <thead>
          <tr>
            <th class="sticky top-0 bg-blueGray-200 font-semibold text-left">
              <div class="w-full h-full p-2 border-b-2 border-blueGray-400">Name</div>
            </th>
            <th class="sticky top-0 bg-blueGray-200 font-semibold text-left">
              <div class="w-full h-full p-2 border-b-2 border-blueGray-400">Username</div>
            </th>
            <th class="sticky top-0 bg-blueGray-200 font-semibold text-left">
              <div class="w-full h-full p-2 border-b-2 border-blueGray-400">Email</div>
            </th>
            <th class="sticky top-0 bg-blueGray-200 font-semibold text-left w-1/5">
              <div class="w-full h-full p-2 border-b-2 border-blueGray-400">Status</div>
            </th>
        ...
        <tbody>
          <tr class="group border-b border-blueGray-200" v-for="user in filtered" :key="user.id">
            <td class="px-2 py-0.5">{{ user.name }}</td>
            <td class="px-2 py-0.5">{{ user.username }}</td>
            <td class="px-2 py-0.5">{{ user.email }}</td>
            <td class="px-2 py-0.5">Offline</td>
            <td class="flex flex-row justify-end items-center space-x-1 px-2 py-0.5 min-h-2.25">
              <button
                ...
                @click="showModal(true, user.deleteUrl)"
              >
    ...
    <confirmation-modal :show="isOpenModal" @close="showModal(false)">
      ...
      <template #footer>
        ...
        <btn class="ml-2" @click="deleteUser">Delete</btn>
      </template>
    </confirmation-modal>
  </app-layout>
</template>
