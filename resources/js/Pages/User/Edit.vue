<template>
  <app-layout>
    <template #title v-if="!!selectedUser">Edit User</template>
    <template #title v-else>Add New User</template>
    ...
    <form class="flex flex-col flex-grow" @submit.prevent="submit">
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <jet-input id="name" type="text" class="mt-1 block w-full" v-model="form.username" required autofocus />
          <div class="mt-0.5 text-rose-700 text-xs" v-if="errors.username">{{ errors.username }}</div>
        </div>
        <div v-if="!!!selectedUser">
          <jet-label for="password" value="Password" />
          <jet-input id="password" type="text" class="mt-1 block w-full" v-model="form.password" required autofocus />
          <div class="mt-0.5 text-rose-700 text-xs" v-if="errors.password">{{ errors.password }}</div>
        </div>
        <div>
          <jet-label for="firstName" value="First name" />
          <jet-input id="firstName" type="text" class="mt-1 block w-full" v-model="form.firstName" required />
          <div class="mt-0.5 text-rose-700 text-xs" v-if="errors.firstName">{{ errors.firstName }}</div>
        </div>
        <div>
          ...
          <div class="mt-0.5 text-rose-700 text-xs" v-if="errors.lastName">{{ errors.lastName }}</div>
        </div>
        <div>
          <jet-label for="email" value="Email Addresss" />
          <jet-input id="email" type="email" class="mt-1 block w-full" v-model="form.email" required />
          <div class="mt-0.5 text-rose-700 text-xs" v-if="errors.email">{{ errors.email }}</div>
        </div>
        <div>
          <jet-label for="phoneNumber" value="Telephone Number" />
          <jet-input id="phoneNumber" type="tel" class="mt-1 block w-full" v-model="form.phoneNumber" />
        ...
        <div>
          ...
          <div class="mt-0.5 text-rose-700 text-xs" v-if="errors.select">{{ errors.select }}</div>
        </div>
        <div>
        ...
        <div>
          <jet-label for="roleId" value="Role" />
          <select
            id="roleId"
            class="mt-1 block w-full border-blueGray-300 focus-within:border-indigo-300 focus-within:ring focus-within:ring-indigo-200 focus-within:ring-opacity-50 rounded-md shadow-sm"
            v-model="form.roleId"
          ...
          <div class="mt-0.5 text-rose-700 text-xs" v-if="errors.roleId">{{ errors.roleId }}</div>
        </div>
        <!-- <div v-if="!!selectedUser">
          <jet-label for="qrcheck" value="Allow QR" />
          <input 
              id="qrcheck"
              type="checkbox" 
              @change="changeStatus($event)" 
              :data-id="selectedUser.id" 
              v-model="selectedUser.allowQR"  
              true-value="1"
              false-value="0">
        </div> -->

        <div>
          <jet-label for="statusId" value="Status" />
          <select
            id="statusId"
            class="mt-1 block w-full border-blueGray-300 focus-within:border-indigo-300 focus-within:ring focus-within:ring-indigo-200 focus-within:ring-opacity-50 rounded-md shadow-sm"
          ...
          <div class="mt-0.5 text-rose-700 text-xs" v-if="errors.statusId">{{ errors.statusId }}</div>
      ...
      <div class="mt-8 flex flex-row justify-start items-center">
        <btn
          class="text-xs rounded-l-md rounded-r-md"
          :variant="kind === 'Assign User to Project' ? 'primary' : 'secondary'"
          @click="kind = 'Assign User to Project'"
        >
          Projects
        </btn>
      </div>
          ...
          <thead>
            <tr>
              <th class="sticky top-0 bg-blueGray-200 font-semibold text-left w-1/2">
                <div class="w-full h-full p-2 border-b-2 border-blueGray-400">Project</div>
              </th>
              <th class="sticky top-0 bg-blueGray-200 font-semibold text-left w-1/2">
                <div class="w-full h-full p-2 border-b-2 border-blueGray-400">Allow QR</div>
              </th>
  ...
  data() {
    return {
      ...
      form: this.$inertia.form({
        ...
        password: '',
        firstName: this.selectedUser ? this.selectedUser.first_name : '',
        lastName: this.selectedUser ? this.selectedUser.last_name : '',
        email: this.selectedUser ? this.selectedUser.email : '',
        phoneNumber: this.selectedUser ? this.selectedUser.phone : '',
        companyId: this.selectedUser ? this.selectedUser.company_id : '',
        address: this.selectedUser ? this.selectedUser.address : '',
        roleId: this.selectedUser ? this.selectedUser.role_id : '',
        statusId: this.selectedUser ? this.selectedUser.status_id : '',
      }),
    };
  },
  ...
  methods: {
    ...
    changeStatus(event){
      ...
      var status = event.target.checked;
       axios.post("/api/project/update/QRaccess", { id: id,status:status})
